...
def translate_text(text, target_lang):
 ...
 prompt = f"Translate this text to {target_lang}. Respond with only the translated text:\n{text}" response = model.generate_content(prompt)
...
with st.sidebar:
 ...
 selected_lang = st.text_input("Enter any language name:", "English")
...
if uploaded_file:
 ...
 st.write(T(result.get("confidence_reason", "AI is uncertain about this diagnosis st.write("**Recommendation:** " + result.get("image_quality_tips", "Provide a cl st.markdown('</div>', unsafe_allow_html=True)
 ...
 """, unsafe_allow_html=True)
 st.write(T(result.get("similar_conditions"))
 st.markdown("</div>", unsafe_allow_html=True)
